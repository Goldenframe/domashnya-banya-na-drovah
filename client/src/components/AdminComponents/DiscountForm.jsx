...
export default function DiscountForm() {
  const { discounts, setCurrentDiscount, fetchActiveDiscounts, userId, token } =
    useOutletContext();
  ...
  const [discountType, setDiscountType] = useState(
    discountData.discount_type || "discount"
  );
  const [applicableServices, setApplicableServices] = useState(
    discountData.applicable_services || []
  );
  const [servicePrices, setServicePrices] = useState(
    discountData.service_prices || {
      broom: 150,
      towel: 200,
      hat: 50,
      sheets: 200,
  ...
  const [freeServiceCounts, setFreeServiceCounts] = useState(
    discountData.free_service_counts || {
      broom: 0,
      towel: 0,
      hat: 0,
      sheets: 0,
  ...
  const [minServiceCounts, setMinServiceCounts] = useState(
    discountData.min_service_counts || { broom: 0, towel: 0, hat: 0, sheets: 0 }
  );
  const [additionalServices, setAdditionalServices] = useState(
    discountData.additional_services || {
  ...
  const [timeDiscountType, setTimeDiscountType] = useState(
    discountData.time_discount_type || "no_time_limit"
  );
    ...
    if (discountData.id) {
      if (discountData.discount_type === "discount_service") {
        setServicePrices((prevPrices) => {
          ...
          return JSON.stringify(prevPrices) !== JSON.stringify(newPrices)
            ? newPrices
            : prevPrices;
        });
        ...
        setFreeServiceCounts((prevCounts) => {
          ...
          return JSON.stringify(prevCounts) !== JSON.stringify(newCounts)
            ? newCounts
            : prevCounts;
        });
      ...
      setAdditionalServices((prevServices) => {
        return JSON.stringify(prevServices) !==
          JSON.stringify(updatedAdditionalServices)
  ...
  const createDiscount = async (newDiscount) => {
    try {
      const response = await axios.post(
        `https://api.dom-ban-na-drovah.ru/api/adminAccount/${userId}/discounts`,
        newDiscount,
        {
          headers: {
  ...
  const updateDiscount = async (discountId, updatedDiscount) => {
    try {
      const response = await axios.put(
        `https://api.dom-ban-na-drovah.ru/api/adminAccount/${userId}/discounts/${discountId}`,
        updatedDiscount,
        {
  ...
  const handleAddDiscount = async (e) => {
    ...
    const newDiscount = {
      ...
      timeDiscountType,
      discountPercentage:
        discountType === "discount" ? parseFloat(discountPercentage) : null,
      applicableServices: Object.keys(additionalServices).filter(
        (service) => additionalServices[service]?.selected
      ),
    ...
    try {
      ...
      showError(
        "Ошибка при добавлении или обновлении скидки. Попробуйте позже."
      );
    }
  };
  const clearForm = () => {
  ...
  allDaysInMonth.forEach((date) => {
    ...
    const isDiscounted = discounts.some((discount) =>
      discount.applicable_days.includes(formattedDate)
    );
  ...
  const isFormValid =
    ...
    ((discountType === "discount" ? discountPercentage : true) ||
      ((discountType === "discount_service"
        ? Object.values(servicePrices).some((price) => price > 0)
        : true) &&
        (discountType === "free"
  ...
  return (
    <div className="account-form">
      ...
      <form onSubmit={handleAddDiscount}>
        ...
        <div className="discount-label">
          <label className="form-label">
            ...
            <button
              type="button"
              className={`mode-btn ${
                timeDiscountType === "no_time_limit" ? "selected" : ""
              }`}
            ...
            <button
              type="button"
              className={`mode-btn ${
                timeDiscountType === "time_limit" ? "selected" : ""
              }`}
        ...
        <div className="form-wrapper">
          ...
          <div
            className={`time-controls ${
              timeDiscountType !== "time_limit" ? "align-left" : ""
            }`}
          >
        ...
        <div className="discount-label">
          <label className="form-label">
            ...
            <button
              type="button"
              className={`mode-btn ${
                discountType === "discount" ? "selected" : ""
              }`}
            ...
            <button
              type="button"
              className={`mode-btn ${
                discountType === "discount_service" ? "selected" : ""
              }`}
                      ...
                      <label className="service-item">
                        <div className="free-service-item">
                          ...
                          <div className="free-service-quantity">
                            <label>
                              Минимальное количество для бесплатной услуги:
                            </label>
        ...
        {!isFormEmpty && (
          <div>
            <p className="auth-cancel" onClick={clearForm}>
              Отменить изменения
            </p>
          </div>
        )}
